...
export default function SideBar() {
  ...
  return (
    <div className="bg-[#212121] w-[15%] h-full overflow-x-hidden overflow-y-scroll pt-[10px]">
      <ul className="flex flex-col bg-[#212121] ">
        ...
        {SideBarData2.map((value, id) => (
          <li key={id} className="flex justify-evenly items-center py-[20px] h-[50px]  hover:bg-[#303030] cursor-pointer">
            <Image src={value.avatar} width={30} height={30} alt={id} />
        ...
        {SideBarData3.map((value, id) => (
          <li key={id} className="flex justify-center items-center py-[20px] h-[50px]  hover:bg-[#303030] cursor-pointer">
            <Image src={value.icon} width={30} height={30} className="basis-[30%] w-[30px] h-[30px]" alt={id} />
            <div className="text-[#FFFFFF] basis-[70%]">{value.title}</div>
          </li>
        ))}
